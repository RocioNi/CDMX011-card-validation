...
const secondBoton = document.getElementById("register2"); //segundo boton,
 boton.addEventListener("click", function () {
  ...
  if (CreditCardNumber.length < 13) {
    //restriccion de cantidad de digitos de la tarjeta
    alert("Favor de introducir un numero de tarjeta, minimo 13 digitos"); // alerta
   } else {
    const resultado = validator.isValid(CreditCardNumber); //  algoritmo
    ...
    if (resultado == true) {
      //condicional , si es true realiza lo siguiente
      document.getElementById("cardNumber").value = hash; //oculta el numero de tarjeta en input al regresar a pantalla 1
      document.getElementById(
        "message"
      ).innerHTML = `<span class="valid">Tarjeta valida </spam>`; // usamos span al tener un solo id message en HTML.
...
secondBoton.addEventListener("click", function () {
  //segundo boton para registrar otra tarjeta, manda a primer pantalla
